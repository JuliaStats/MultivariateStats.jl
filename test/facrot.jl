...
@testset "Factor Rotations" begin
    ...
    FR = rotate(X, CrawfordFerguson{Orthogonal}(kappa=1.0 / 5.0))
    @test isapprox(loadings(FR), F, rtol=1e-6)
    @test isapprox(rotation(FR), R, rtol=1e-6)

    FR = rotate(X, Oblimin{Orthogonal}(gamma=1.0))
    @test isapprox(loadings(FR), F, rtol=1e-6)
    @test isapprox(rotation(FR), R, rtol=1e-6)
    ...
    FR = rotate(X, CrawfordFerguson{Orthogonal}(kappa=p / (2 * d)))
    @test isapprox(loadings(FR), F, rtol=√eps(Float64))
    ...
    FR = rotate(X, CrawfordFerguson{Orthogonal}(kappa=(p - 1) / (d + p - 2)))
    @test isapprox(loadings(FR), F, rtol=√eps(Float64))
