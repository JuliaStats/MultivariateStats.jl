...
.. function:: fit(MulticlassLDA, nc, X, y; ...)
    ...
    **Keyword arguments:**
    ...
    ----------------- --------------------------------------------------------------- ------------------------
     covestimatorSb   Custom covariance estimator for between-class covariance.        ``SampleCovariance()``
                      The covariance matrix will be calculated as
                      ``cov(covestimatorSb, #=data=#; dims=2, mean=zeros(#=...=#)``.
    ...
    ----------------- --------------------------------------------------------------- ------------------------
     covestimatorSw   Custom covariance estimator for within-class covariance.         ``SampleCovariance()``
                      The covariance matrix will be calculated as
                      ``cov(covestimatorSw, #=data=#; dims=2, mean=zeros(nc))``.
                      Custom covariance estimators, available in other packages,
                      may result in more robust discriminants for data with more
                      features than observations.
    ================= =============================================================== ========================
